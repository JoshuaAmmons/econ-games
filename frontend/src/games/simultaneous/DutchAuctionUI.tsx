...
/**
 * Dutch Auction UI
 * Price descends — players submit the price at which they'd stop the clock.
 * Strategically equivalent to first-price sealed bid: winner pays own price.
 */
const DutchAuctionUI: React.FC<GameUIProps> = ({
  session: _session,
  player,
  playerId,
  roundId,
  ...
  const isBiddingAboveValue = stopPrice !== '' && priceNum > privateValue;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    submitAction({ type: 'decision', stopPrice: val });
    setSubmitted(true);
      ...
      <div className="space-y-4">
        <Card>
          <div className="text-center mb-3">
            <Gavel className="w-8 h-8 mx-auto text-amber-600 mb-1" />
            <div className="text-sm text-gray-500">Dutch Auction</div>
            <div className="text-lg font-bold text-amber-700">Descending Clock</div>
          </div>
          <div className="text-xs p-2 rounded text-center bg-amber-50 text-amber-700">
            First-price rule: winner pays their own stop price
          </div>
        ...
        <Card title="Set Your Stop Price">
          {roundActive && roundId ? (
            submitted ? (
              <div className="text-center py-4">
                <div className="text-green-600 font-medium mb-2">Stop Price Submitted!</div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <Input
                  ...
                  placeholder="Price to stop the clock"
                  required
                />
                {isBiddingAboveValue && (
                  <div className="bg-red-50 border border-red-200 rounded p-2 text-xs text-red-700">
                    <span className="font-medium">Warning:</span> Stop price above your value risks a loss!
                  </div>
                )}
                ...
                <Button type="submit" className="w-full" disabled={submitting || !stopPrice}>
                  {submitting ? 'Submitting...' : 'Submit Stop Price'}
                </Button>
              </form>
            )
          ) : (
        ...
        <Card title="Auction Results">
          {results ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          Stop Price: ${Number(r.stopPrice).toFixed(2)}
            ...
            <div className="text-center text-gray-400 py-8">
              ...
              <p>Results will appear after all stop prices are submitted</p>
